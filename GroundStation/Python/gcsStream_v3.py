...
model_path = r"C:\development\NVIDIA_brev_files\train_results\final_weights\train7\weights\best.pt"
...
class ProcessedStreamThread(QThread):
    ...
    def connect_to_stream(self) -> bool:
        try:
            ...
            # Force TCP transport for RTSP reliability
            import os
            ...
            if not self.cap.isOpened():
                self.stream_status.emit(self.stream_id, False, "Failed to open RTSP stream")
                return False
    ...
    def read_latest_frame(self):
        ...
        try:
            # Drain buffer to get latest frame — critical for RTSP latency
            for _ in range(4):
        ...
        while self.running:
            try:
                if not self.connect_to_stream():
                    ...
                    if self.consecutive_failures >= RTSP_MAX_FAILURES:
                        self.stream_status.emit(self.stream_id, False, f"Max failures reached ({RTSP_MAX_FAILURES})")
                        time.sleep(RTSP_RECONNECT_TIMEOUT * 2)
                    else:
                        time.sleep(RTSP_RECONNECT_TIMEOUT)
                    continue
...
class ProcessedMultiStreamViewer(MultiStreamViewer):
    ...
    def add_stream(self, stream_url: str, stream_id: int, drone_name: str) -> bool:
        ...
        drone_map = {
            "M300RTK_A": DJI_M300RTK_A,
            "M300RTK_B": DJI_M300RTK_B,
            "M300RTK_C": DJI_M300RTK_C,
            "M300RTK_D": DJI_M300RTK_D,
            "M300RTK_E": DJI_M300RTK_E,
            "M4T": DJI_M4T,
            "M3E": DJI_M3E,
            "M400": DJI_M400,
        }

        if drone_name not in drone_map:
            print(f"Unknown drone name: {drone_name}")
            return False

        drone = drone_map[drone_name]
        drone_interface = DJIInterfaceLite(drone["IP_RC"])

        thread = ProcessedStreamThread(stream_url, stream_id, self.inference_worker, drone_interface, drone)
        thread.frame_ready.connect(self.update_stream)
        thread.stream_status.connect(self.handle_stream_status)
